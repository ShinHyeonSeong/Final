...
public class UserController {
    ...
    public String save(@RequestParam("email") String email,
        ...
        UserDto findUser = userService.findUserByEmail(email);
        if (findUser == null) {
    ...
    public String login(@RequestParam("email") String email,
        ...
        UserDto loginResult = userService.signIn(email, password);
        if (loginResult != null) {
    ...
    public String goAccount(HttpSession session, Model model) {
        ...
        UserDto result = userService.findUser(sessionUser.getUuid());
...
//        }
    ...
    public String update(@RequestParam("email") String email,
        ...
        UserDto updateDto = userService.updateUser(sessionUser, email, name);
...
//            session.setAttribute("userInfo", updateDto);
    ...
    public String passwordChange(@RequestParam("email") String email,
        ...
        int result = userService.updatePassword(sessionUser, email, password, newPassword, confirmPassword);
        if (result == 0) {
            session.removeAttribute("userInfo");
            return "login";
        } else return "login";
    ...
    public String deleteById(HttpSession session) {
        UserDto userDto = getSessionUser();
        userService.deleteUser(userDto.getUuid());
        session.invalidate();
