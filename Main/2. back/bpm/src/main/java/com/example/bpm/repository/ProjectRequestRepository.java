...
public interface ProjectRequestRepository extends JpaRepository<ProjectRequestEntity, ProjectRequestPKEntity> {
    @Query(// 괄호한 value 값의 sql문을 실행시킴
            value = "insert into  " +
                    "projectRequst " +
                    "values (:sendUUID, :recvUUID)", //파라미터의 변수를 활용하고 싶으면 : 를 붙여라 / ?1 이렇게 하면 1번째 파라미터를 쓰겠다는 의미
            nativeQuery = true)
        //sql문이 작동하게 도와줌
    Optional<ProjectRequestEntity> plusProject(String sendUUID, String recvUUID);

    //권한자 프로젝트를 확인할 수 있는 메서드
    @Query(value = "select * from projectRequst where" +
            "projectRequst.sendUUID = :sendUUID",
            nativeQuery = true)
    ProjectRequestEntity selectToHostList(String sendUUID);
